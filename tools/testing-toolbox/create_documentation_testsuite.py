...
def check_prerequisites():
    ...
    if not 'TEST_SCRIPT' in os.environ:
        print("Error: Please supply TEST_SCRIPT as an environment variable.")
        exit(1)
    ...
    test_script = read_env_input("TEST_SCRIPT")
...
def read_templates():
    ...
    with open ("/templates/test-docs.sh.j2", "r") as f:
...
def write_test_script(testsuite, test_params):
    with open ('/target/test.sh', 'w') as f:
        f.write(test_script_template.render( { 'testsuite': testsuite, 'test_script': test_script, 'git_branch': git_branch, 'test_params': test_params }))
        f.close()
...
def create_testsuite():
    ...
    # no operators in the documentation tests, because the tests install them on their own
